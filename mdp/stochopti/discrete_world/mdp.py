...
class MDP:
   ...
   Methods
    ...
    def __init__(self, space: finiteTimeSpace, **kwargs):
        ...
        self.A = space.action_set
        self.adm_A = space.admisible_actions
        self.l_A = space.l_A
        self.A_int = space.A_int
        self.int_A = space.int_A

        self.Q = space.transition_kernel
        self.r = space.reward
...
class MDPLPSolver:
    ...
    def _build_sparse_A_matrix(self):
        ...
        for i in range(size):
            ...
            if a in self.mdp.space.admisible_actions(j):
...
class gurobiMDPSolver(MDPLPSolver):
    ...
    def build_linear_program(self):
        ...
        model.addConstrs(sum(x[j, a] for a in self.space.admisible_actions(j))
                         - sum(self.lambda_ * self.space.transition_kernel(s, a)[j] * x[s, a]
                               for s in self.S for a in self.space.admisible_actions(s)
                               if j in self.space.transition_kernel(s, a).keys())
